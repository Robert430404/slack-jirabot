...
class Bot {
  ...
  issueResponse (issue, usrFormat) {
    ...
    response.text = ResponseTransformer.transformDescription(issue.fields.description);
    ...
    if (this.config.disableFootnote) {
      response.footer = '';
    }
    ...
    if (format === RESPONSE_FULL) {
      ResponseTransformer.transformFullResponse(issue, response, this.config.usermap);
    ...
    if (format !== RESPONSE_FULL) {
      ResponseTransformer.transformMinimalResponse(issue, response);
    }
